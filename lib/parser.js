...
const VBKParser = (() => {
    ...
    if (noteMatch) {
      ...
      if (note) {
        const cleaned = note.split('\n')
          .filter(l => !l.includes('系统信息') && l.trim() !== '添加商家备注')
          .map(l => l.trim()).filter(Boolean).join('\n');
        if (cleaned) meta.merchant_note = cleaned;
      }
    }
    ...
    for (const table of tables) {
      ...
      if (headerText.includes('资源名称') && headerText.includes('出发城市')) {
        ...
        for (let i = 1; i < rows.length; i++) {
          ...
          if (city && city !== '--' && city.length >= 2) return _normalizeCityName(city);
        }
      }
    }
      ...
      if (headerText.includes('出发机场') && headerText.includes('航班号')) {
        ...
        for (let i = 1; i < rows.length; i++) {
          ...
          segments.push({
            direction: i === 1 ? '去程' : '回程',
            flight_no: flightNo,
            from: _normalizeCityName(_mapAirportToCity(from)),
            to: _normalizeCityName(_mapAirportToCity(to)),
            departure_time: departureTime,
            arrival_time: arrivalTime,
            // 去程登记：到达机场+到达时间；返程登记：出发机场+出发时间
            register_airport: _normalizeCityName(_mapAirportToCity(i === 1 ? to : from)),
  ...
  function extractTeamCategory() {
    // ponytail: 匹配 SPAN.ant-tag.tag-item-blue 或任意含 "意向X团" 的元素
    const el = Array.from(document.querySelectorAll('span, div'))
      .find(el => /意向.+?团/.test(el.textContent) && el.textContent.length < 100);
    if (!el) { console.log('[VBK] extractTeamCategory: no element found'); return '快拼团'; }
    console.log('[VBK] extractTeamCategory: found', el.tagName, el.textContent.trim().substring(0, 60));
    // ponytail: [^团]+ 确保 capture 整个词（"青年"而非"青"）
    const m = el.textContent.trim().match(/意向([^团]+)团/);
    if (!m) { console.log('[VBK] extractTeamCategory: regex no match on', el.textContent.trim()); return '快拼团'; }
    console.log('[VBK] extractTeamCategory: captured', m[1].trim());
    const map = { '普通': '快拼团', '老友': '老友团', '青年': '青年团', '亲子': '亲子团' };
    const result = map[m[1].trim()] || m[1].trim() + '团';
    console.log('[VBK] extractTeamCategory: result', result);
    return result;
  }
  ...
  function parse() {
    ...
    const teamCategory = extractTeamCategory();
  ...
  return {
    ...
    waitForDecrypt,
    normalizeCityName: _normalizeCityName,
    extractTeamCategory,
  };
})();