...
const VBKParser = (() => {
  ...
  function parseTravellersFromDOM() {
    ...
    for (let i = 1; i < rows.length; i++) {
      ...
      if (idText) {
        ...
        const idMatch = idText.match(/[\d*]{15,18}/);
  ...
  function _parseTravellerBlock(block) {
    ...
    for (let i = 1; i < parts.length; i++) {
      ...
      if (p === '男' || p === '女') {
        ...
        // 可能含证件号： "身份证 • 1201***********542"
        const idMatch = p.match(/[\d*]{15,18}/);
        if (idMatch) idNo = idMatch[0];
      } else if (/\d{15,18}/.test(p) || /[\d*]{15,}/.test(p)) {
        const idMatch = p.match(/[\d*]{15,18}/);
        if (idMatch && !idNo) idNo = idMatch[0];
  ...
  function extractRoomInfo(note, travellers, routeName) {
    ...
    if (!travellers || travellers.length === 0) {
      return { star_rating: starRating, biao: 0, dachuang: 0 };
    }
    ...
    return { star_rating: starRating, biao, dachuang };
  }
  ...
  function _extractIdNo(text) {
    if (!text) return '';
    // 匹配身份证号（可能脱敏）：1201***********542 或完整18位
    const m = text.match(/[\d*]{15,18}/);
